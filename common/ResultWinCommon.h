...
{
protected:
	typedef std::list<Search::ResultEntry> result_list;
	result_list results;
	int view_lines;
...
public:
	...
	void appendResult( const Search::Result & result, const std::wstring & path, bool do_append = true, Search::ResultEntry *address = NULL );

	virtual void append( const std::wstring & path, bool use_icon, Search::ResultEntry *address ) = 0;
	virtual void append( const std::vector<std::wstring> & paths, bool use_icon, Search::ResultEntry *address ) = 0;
